...
def get_allowed_origins(env) -> set[str]:
    configured = get_secret(env, "ALLOWED_ORIGINS")
    ...
    # Useful for local static-server development.
    origins.update(
        {
            "http://localhost:8000",
            "http://127.0.0.1:8000",
...
async def add_cors_headers(request: Request, call_next):
    ...
    if origin and origin in allowed_origins:
        ...
        response.headers["Access-Control-Allow-Headers"] = "Content-Type, X-Ingest-Secret"
